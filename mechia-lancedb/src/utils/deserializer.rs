...
use mechia::vector_store::VectorStoreError;
use serde::Serialize;
use serde_json::{json, Value};

use crate::serde_to_mechia_error;

fn arrow_to_mechia_error(e: ArrowError) -> VectorStoreError {
...
fn type_matcher(column: &Arc<dyn Array>) -> Result<Vec<Value>, VectorStoreError> {
    match column.data_type() {
        ...
        DataType::RunEndEncoded(index_type, ..) => {
            ...
            items
                .iter()
                .map(|item| serde_json::to_value(item).map_err(serde_to_mechia_error))
                .collect()
        }
        DataType::BinaryView
        | DataType::Utf8View
    ...
    I: Iterator<Item = Result<Vec<Value>, VectorStoreError>>,
{
    fn map_ok(self) -> Result<Vec<Value>, VectorStoreError> {
        self.map(|maybe_list| match maybe_list {
            Ok(list) => serde_json::to_value(list).map_err(serde_to_mechia_error),
            Err(e) => Err(e),
        })
        .collect::<Result<Vec<_>, _>>()
    }
