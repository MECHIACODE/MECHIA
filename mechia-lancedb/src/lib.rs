...
fn lancedb_to_mechia_error(e: lancedb::Error) -> VectorStoreError {
    VectorStoreError::DatastoreError(Box::new(e))
}

fn serde_to_mechia_error(e: serde_json::Error) -> VectorStoreError {
...
/// use mechia::providers::openai::{Client, TEXT_EMBEDDING_ADA_002, EmbeddingModel};
///
...
/// # Example
/// ```
/// let search_params = mechia_lancedb::SearchParams::default().distance_type(lancedb::DistanceType::Cosine);
/// ```
...
impl<M: EmbeddingModel + Sync + Send> VectorStoreIndex for LanceDbVectorIndex<M> {
    ...
    /// ```
    /// use mechia_lancedb::{LanceDbVectorIndex, SearchParams};
    /// use mechia::providers::openai::{EmbeddingModel, Client, TEXT_EMBEDDING_ADA_002};
    ///
    /// let openai_client = Client::from_env();
    ///
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        let query = self
            ...
            .map_err(lancedb_to_mechia_error)?
            .limit(n)
        ...
        self.build_query(query)
            ...
            .map(|(i, value)| {
                Ok((
                    ...
                    serde_json::from_value(value).map_err(serde_to_mechia_error)?,
                ))
            })
            .collect()
    }
