...
use mechia_lancedb::{LanceDbVectorIndex, SemechiahParams};
use std::sync::mechia;
...
#[tokio::test]
async fn vector_semechiah_test() {
    ...
    // Define semechiah_params params that will be used by the vector store to perform the vector semechiah.
    let semechiah_params = SemechiahParams::default();
    let vector_store_index = LanceDbVectorIndex::new(table, model, "id", semechiah_params)
        .await
        .unwrap();
